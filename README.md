...
### 🔴🔴 กฎเหล็ก: สคริปต์ทับศัพท์ UGC Speaker — ห้ามแก้ (Mark โดยเจ้าของงาน 2026-08-30)

> **สคริปต์ทับศัพท์ผ่านแล้ว (ตัดคำ/ทับศัพท์ดีมาก) เจ้าของงาน Mark ไว้ว่าห้ามแก้**
> ใช้ผลลัพธ์ปัจจุบันเป็น baseline ที่ถูกต้อง ส่งต่องานโดยตรง ห้ามแตะโดยพลการ

**ผลลัพธ์ที่ถือว่าถูกต้อง (baseline):**
- ทับศัพท์ชื่อครบทุกคำ (Dr.PONG → "ดร.พงษ์ ยูเก้าจุดเก้า รีนิวเวิลด์ ครีม ห้าสิบกรัม")
- ตัดคำให้พอดีกับระยะโฆษณา ไม่เหลือแค่ "ดร." — ชื่อ/แบรนด์ไม่ถูกชำแหละ
- ไม่มีคำซ้ำ (dedupe ไทยแบบติดกัน + ส่วนท้าย; เก็บ non-adjacent ซ้ำไว้)
- PA+++/PA++++ → "พี เอ พลัส" · SPF50+ → "เอส พี เอฟ ห้าสิบ พลัส"
- ตัวเลข/หน่วยเป็นไทย (7.5g → เจ็ดจุดห้ากรัม, 30ml → สามสิบมิลลิลิตร)

**ห้ามแก้โดยไม่ได้รับคำสั่ง — โค้ดต่อไปนี้:**
- `prompt-builder-service/prompt_builder.py`: `_tts_product_name` (~1797), `_owner_script_variants` (~2085), `_drop_later_name_mentions` (~2133), `_fit_beat_text` (~2221)
- `modules/video/script_gen.py`: `_brand_tokens`, `_dedupe_product_name`
- `modules/video/pipeline_affiliate.py`: ส่วน thai_script (~1111)

**ถ้าจะแก้จริง (เช่น จุดค้าง "7.5g" ไม่โผล่ใน script เต็มที่โดนกรองขั้น `product_short_for_tts`/`_shorten") → ต้องคุยกับเจ้าของงานก่อนเสมอ** ห้ามเดา/แก้เอง/ยิงงานใหม่ให้เปลือง
