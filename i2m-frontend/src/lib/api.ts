...
export const api = {
  ugc: {
    generateScript: async (data: { productName: string; productDesc: string }) =>
      (await fetch(`${UGC_API}/scripts/generate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        product_name: data.productName,
        product_desc: data.productDesc,
      }) })).json(),
    generateUgcScript: async (data: { productName: string; productDesc: string; style?: string }) =>
      (await fetch(`${UGC_API}/scripts/ugc`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        product_name: data.productName,
        product_desc: data.productDesc,
  ...
  image: {
    generateEtsy: async (data: { productName: string; description?: string; style?: string }) =>
      (await fetch(`${ETZY_API}/ai/generate-image`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        product_name: data.productName,
