...
async def _extract_gender_age_hashtags(title: str, description: str, category: str = "") -> dict:
    ...
    prompt = (
        ...
        "- Decide body_part by WHAT BODY PART the product is actually applied to. Keywords tell you the applied site: ครีมทาหน้า/ผิวหน้า/ใบหน้า/face/serum/cream → face; body lotion → body; belly firming → belly; hair → hair; hands → hands; nails → nails; lips → lips; whole body wash/shower → whole-body; empty if not body-applicable.\n"
        "- A 'pregnant/ท้อง/คุณแม่/maternity' label is about WHO the buyer/user is, NOT automatically where it is applied. So a face cream for pregnant women (ครีมทาหน้าคนท้อง) = body_part=face, special_target=pregnant — do NOT default to belly just because 'คนท้อง' appears.\n"
        "- Only use body_part=belly + special_target=pregnant when the product is genuinely applied TO the belly during pregnancy, e.g. anti-stretch-mark cream, belly firming oil, ทาหน้าท้อง, กันรอยแตกลาย.\n"
        "- Always fill body_part from what the product actually is (the applied site), never from the audience label. Leave empty only if truly not body-applicable.\n"
