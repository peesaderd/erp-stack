...
def generate_video(
    ...
    if thai_voice_mode:
        ...
        # 🔴 FIX (owner 2026-08-30): review/product_demo ต้อง "วางบนโต๊ะ มือนิ่ง ไม่ถือ"
        # ไม่ใช่ถือสินค้าไว้ในมือ — เดิม hardcode "ถือสินค้าไว้ในมือ" ตลอดในโหมด Wan พูดเอง
        # ทำให้ review ออกมาเป็นถือครีมเสมอ แม้ prompt-builder จะให้ prompt วางโต๊ะแล้ว
        _pw_is_review = ugc_style in ("review", "product_demo")
        if _pw_is_review:
            _pw_body_line = (
                f"นั่งอยู่ตรงหน้ากล้อง วางสินค้าไว้บนโต๊ะตรงหน้า มือวางนิ่ง ๆ บนโต๊ะ ไม่ถือสินค้า "
                f"ไม่หยิบขึ้นมา ไม่ทาครีม ไม่แตะต้องใบหน้า "
                f"สินค้าอยู่บนโต๊ะตรงหน้ากล้องชัดเจน ฉากหลังเบลอ นิ่ง กล้องนิ่ง ไม่ซูม ไม่ขยับ "
            )
        else:
            _pw_body_line = (
                f"ผู้หญิงไทยอายุยี่สิบปี ยืนตรงหน้ากล้อง ถือสินค้าไว้ในมือ ระดับหน้าอก หันหน้าตรง "
                f"สินค้าอยู่ตรงกลางภาพชัดเจน ฉากหลังเบลอ นิ่ง กล้องนิ่ง ไม่ซูม ไม่ขยับ "
            )
        final_prompt = (
            _pw_body_line +
            ...
            f"ขยับปากให้ตรงกับเสียงพูดภาษาไทยทุกคำในระหว่างที่พูดเท่านั้น "
            f"พูดบทจบแล้วให้หยุดพูดทันที ปิดปากสนิท ไม่มีคำเติมท้าย ไม่มีเสียงใด ๆ ต่อจากประโยคสุดท้าย "
            f"แล้วยิ้มและถือสินค้าให้นิ่ง ๆ ตรงหน้ากล้องต่อไปอีก 3 วินาทีโดยไม่ขยับ ไม่พูด ไม่ขยับปาก "
