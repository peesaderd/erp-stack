...
def _deepseek_product_prompts(product_name: str, description: str, ugc_style: str = "holding",
    ...
    try:
        ...
        sysprompt = (
            ...
            "{\n"
            " \"thai_script\": \"<Natural spoken Thai hook + benefit + CTA, ~120-150 characters>\",\n"
...
def generate_video(
    ...
    if thai_voice_mode:
        ...
        if _motion_on and _motion_txt:
            _motion_block = (
                f"\n(หลังอ่านบทจบ ขณะที่ยังไม่ปิดกล้อง ให้ขยับตัว/มือ/สีหน้าต่อตามนี้จริง ๆ "
                f"ห้ามอ่านออกเสียงท่อนนี้เด็ดขาด เพราะเป็นคำสั่งท่าทาง ไม่ใช่บทพูด):\n{_motion_txt}\n"
            )
        final_prompt = (
        ...
        logger.info(f"  🎙 Voice mode A: บทพูดไทยล้วน + motion {'AFTER-script' if _motion_block else 'OFF'} (owner fix 2026-09-10 05:38, len={len(final_prompt)}, motion={len(_motion_block)}ch)")
