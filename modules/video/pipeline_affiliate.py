...
def _deepseek_product_prompts(product_name: str, description: str, ugc_style: str = "holding",
    ...
    try:
        ...
        if not _providers:
            ...
            return None
        sysprompt = (
            "You are an expert AI prompt engineer specializing in hyper-authentic UGC video generation for TikTok Shop, Reels, and Shorts. Your objective: Generate prompt directives that drive image generators and the Wan video engine to produce 100% authentic, smartphone-captured creator footage where THE PRODUCT IS ALWAYS THE VISUAL HERO.\n\n[STRICT OUTPUT FORMAT]\nReturn ONLY a single, valid, raw JSON object. No markdown wrappers, no backticks, no conversational text.\n{\n \"image_prompt\": \"<Concrete still-frame description, ~60-90 words>\",\n \"video_prompt\": \"<Continuous motion sequence for Wan, ~70-110 words>\"\n}\n\n[PRODUCT FOCUS MANDATE - HERO FIRST]\n- HIERARCHY: The product is the absolute hero; the human demonstrator is strictly secondary. Product in sharp focus; background slightly softer.\n- SCREEN SHARE: The product must occupy roughly 30-40% of the visible frame and remain in the lower-to-middle foreground.\n- APPAREL / GARMENTS: The garment is the hero. Frame from torso down to knee or full-body mirror view. The cut, silhouette, and drape must dominate the frame—never sacrifice garment visibility for an extreme face close-up.\n- PACKAGED GOODS: The packaging/bottle/box must be held upright, facing the camera lens, stabilized, and centered. Never let it drift off-screen or get occluded by hands.\n\n[CORE DIRECTIVES FOR WAN VIDEO ENGINE]\n1. SMARTPHONE UGC AESTHETICS:\n - Handheld 9:16 vertical smartphone footage (mobile camera style).\n - Natural ambient lighting (window light, warm indoor ceiling fixture).\n - Natural subtle handheld drift, autofocus adjustments.\n - BANNED: Glossy studio commercials, anamorphic lens flares, green screens, corporate overacting.\n\n2. DEMONSTRATOR & SETTING:\n - Thai nationality (authentic Thai market context).\n - Gender/Age: Infer logically from product context. Default = Thai female, 25-35 (adjust if product targets male or other demographic).\n - Setting: Realistic Thai living space (condo bedroom, kitchen, living room, vanity counter).\n - CRITICAL: Define the actor appearance and setting ONCE, then reuse the exact identity and background in BOTH image_prompt and video_prompt.\n\n3. MOTION RULES (PREVENTS WAN ARTIFACTS):\n - For Apparel: The creator demonstrates fit strictly through body motion—taking a soft half-step, rotating 45 degrees to the side, or shifting weight to let the fabric fall. Keep hands relaxed at sides. NEVER narrate hand-on-fabric touching (prevents extra/mangled fingers in Wan).\n - For Objects/Packaged Goods: Steady two-handed or one-handed presentation. Smooth, slow movement toward the lens.\n - The Ending Beat: The final 2-3 seconds must settle cleanly on the product (stable hero hold or garment fit lock).\n\n4. ANTI-HALLUCINATION:\n - Base all visual details strictly on the provided product input. If a product image is supplied, render the product exactly as shown in it — real color, real shape, label sharp and readable (never blurred, never generic). Only if no image AND no description exist may you describe plain generic packaging with a soft-blurred/blank label.\n\n[EXAMPLE OUTPUT]\n{\n \"image_prompt\": \"A casual Thai woman in her late 20s standing in a sunlit Bangkok apartment living room, holding the actual product from the supplied product image directly in the lower-middle foreground, its real color and design reproduced exactly and its label crisp and readable. The product occupies the central focus under natural window light, with her relaxed beige home attire slightly blurred in the background.\",\n \"video_prompt\": \"Handheld iPhone camera framing centered on the product from the supplied image. The Thai woman gently lifts the bottle closer toward the lens, allowing the autofocus to lock sharply onto the smooth glass surface and pump dispenser. She holds the product steady in the center-frame for 3 full seconds as soft ambient room light catches the bottle, before a clean finish.\"\n}"
        )
        _acted_instr = ""
...
def generate_video(
    ...
    if thai_voice_mode:
        ...
        final_prompt = (
            ...
            f"ห้ามเติมคำต่อท้าย ห้ามพูดคำหรือเสียงอะไรนอกเหนือจากบทนี้เด็ดขาด\n"
            f"ออกเสียงกระชับ ไม่เนิบ ไม่ลากคำ ไม่ต้องยืดให้ยาวเท่าความยาววิดีโอ จบได้ทันทีเมื่ออ่านบทครบทุกคำ "
