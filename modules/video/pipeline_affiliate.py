...
def _deepseek_product_prompts(product_name: str, description: str, ugc_style: str = "holding",
    ...
    try:
        ...
        sysprompt = (
            ...
            "Reply with the single JSON object alone (keys: image_prompt, video_prompt). Write IMAGE_PROMPT as a concrete, complete "
            "on-model frame description (roughly 50-95 words — detailed enough for the image model to render one good frame). "
            "Write VIDEO_PROMPT as a FULL, detailed continuous motion sequence (roughly 60-105 words — do NOT compress it to a single short "
            "sentence). Give Wan enough shot guidance: who is on camera, what they are wearing/doing, how the product sits in frame and looks "
            "worn or in-use, camera framing + moves + focus, and the ending. A richer, complete video_prompt drives a better video; "
            "do not squeeze detail out to stay short.\n"
            "[CORE UGC RULES]\n"
            ...
            "2. The PRODUCT IS THE HERO — the product stays visually central through the whole clip, never secondary to the face/room. "
            ...
            "[THAI ETHICS & CONTEXT LOCK — owner 2026-09-09] This content is a Thai-language TikTok Shop UGC ad for the Thai market. "
            "The person on camera IS a Thai woman creator and the room/kitchen/home is a real Thai living space. STRICTLY: the demonstrator is Thai "
            "(Thai Southeast-Asian look) and speaks/sells in Thai context — ABSOLUTELY NO foreign nationality, NO random ethnicity, NO non-Thai or Middle-Eastern "
            "or Western or African or Indian or East-Asian-only face, NO changing the person's nationality/ethnicity between image and video or between scenes. "
            "One fixed Thai woman, one Thai setting, whole clip. Do not write nationality/ethnicity words that are NOT Thai. Same Thai woman every frame.\n"
            ...
            "Fix on ONE specific adult THAI woman creator and describe her concretely so the image model is not left to invent a random plain face or ethnicity. "
            "Owner rule 2026-09-09 (Thai ethics): she is THAI (Southeast-Asian Thai look) — vary her face shape/hair/style/outfit WITHIN a Thai look across videos so content is fresh, "
            "but NEVER make her non-Thai and NEVER mix nationalities. "
            "Choose a natural, credible Thai creator that fits THIS product and category: vary age within the target group, face shape, hair, "
            ...
            "NEVER change this identity between image and video, or between first frame and later frames — same Thai woman throughout. "
            ...
            "Write the IMAGE_PROMPT as one natural handheld on-model STILL FRAME where the PRODUCT ITSELF is clearly IN FRAME and the visual hero — "
            "(for apparel the Thai woman is WEARING the garment and it is clearly on her body in focus; for other products the physical product/packaging is held upfront "
            "and fills the lower-middle frame in sharp focus). Do NOT write a vague scenic narration; name concretely what is visible: the Thai woman, one fixed Thai room, "
            "and the actual product type in-frame. "
            "Write the VIDEO_PROMPT as one continuous, natural handheld motion sequence that moves through the real action "
            "(for apparel: pull-on/walk/smooth-worn-fit/tug-waistband; for others: pour/scoop/unbox/feed/dispense) — be detailed and complete, "
            "and reuse the fixed Thai-woman identity line verbatim so image and video describe the SAME Thai woman and SAME product in-frame state.")
        _acted_instr = ""
