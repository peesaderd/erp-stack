...
def _deepseek_product_prompts(product_name: str, description: str, ugc_style: str = "holding",
    ...
    try:
        ...
        sysprompt = (
            ...
            "2. The PRODUCT IS THE HERO — it occupies the visual center in EVERY shot, never secondary to the model or the face. "
            "shoot close-up / mid shot that keeps the product itself (the jeans/garment/apparel texture+wash+tag, or the packaging/label) "
            "large, sharp, front-and-center in the LOWER-to-MIDDLE of frame for the majority of screen time — not the model's face, not the body, not the room. "
            "The creator's hands genuinely hold/spread/show/rotate the product toward the lens so the material/print/fit reads clearly. "
            "The label/waistband/packaging must stay in sharp focus. Product must NEVER be tiny, off to the side, out of frame, or blocked. \n"
            "2b. THE LAST 2-3 SECONDS MUST END WITH A DEVICE-HELD CLOSE-UP OF THE PRODUCT ITSELF facing the camera "
            "(hold the jeans/garment up, smooth the fabric, point at the waistband seam/print, or set the product down center-lens and let the camera linger on it) "
            "so the final beat unmistakably showcases the item being sold. The video must NOT end on a bare face/bust close-up or an empty room shot.\n"
