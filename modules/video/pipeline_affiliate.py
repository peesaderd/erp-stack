...
def generate_video(
    ...
    if thai_voice_mode:
        ...
        # 🔴 FIX (owner 2026-08-30 02:07): เอา hardcode คำสั่งท่าออกทั้งหมด — no hardcoded
        # body/pose/take. ท่าทาง (ถือ/วาง/ทาครีม) ต้องมาจาก SSOT (prompt-builder) เท่านั้น.
        # `prompt` (video prompt ที่ได้จาก prompt-builder) เป็นตัวตั้ง — ฝังแค่บทพูดไทยต่อท้าย
        # เพื่อให้ Wan ขยับปากตามเสียงจริง (rule owner 2026-08-25: ภาษาไทยล้วน).
        final_prompt = (
            prompt + 
            f"พูดบทภาษาไทยต่อไปนี้ออกเสียงดังให้กล้องฟัง คำต่อคำ ไม่ข้ามไม่แต่งเพิ่ม: \"{thai_script}\" "
            f"พูดด้วยน้ำเสียงที่เป็นธรรมชาติ มั่นใจ กระชับ ชัดเจน ไม่รีบร้อน ไม่พูดพล่อย ๆ "
            ...
            f"แล้วยิ้มนิ่ง ๆ ตรงหน้ากล้องต่อไปอีก 3 วินาทีโดยไม่ขยับ ไม่พูด ไม่ขยับปาก "
