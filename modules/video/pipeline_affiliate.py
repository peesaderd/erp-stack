...
def _deepseek_product_prompts(product_name: str, description: str, ugc_style: str = "holding",
    ...
    try:
        ...
        sysprompt = (
            ...
            " \"video_prompt\": \"<ONE continuous shot with start/middle/settle, grounded in physical detail, 40-80 words>\"\n"
            ...
            "[PRODUCT-FIRST VISUAL EXECUTION]\n"
            "- Read the product type first, then demonstrate it the natural way:\n"
            "  * Worn (apparel/underwear): the creator wears it and shows fit and drape with gentle turns and "
            "steps, hands relaxed, letting the garment move naturally.\n"
            "  * Applied (skincare/cosmetics): show a natural application gesture, or hold it cleanly at chest "
            "level facing the lens.\n"
            "  * Consumed / packaged / tools: hold it centered in the lower-mid frame, steady, with the real "
            "label of the reference product showing clearly.\n"
            "- Camera: vertical 9:16 smartphone, natural subtle handheld movement, crisp focus on the product as "
            ...
            "3. ONE CONTINUOUS ACTION with a clear start, middle, and end. Match the motion scale to the clip length \n"
            "   - a longer shot gets ONE fuller motion with a settle at the end, never several separate actions \n"
            "   stacked together. End the shot on a calm, settled beat.\n"
            "4. CAMERA AND LIGHT: carry over the camera angle, lens feel, and light source from your image_prompt, \n"
            "   then add ONE subtle camera behaviour (static, a very slow push-in, or a gentle drift).\n"
            "5. CLIP-LENGTH AWARENESS: a short clip is one small gesture; a longer clip is one fuller motion with a \n"
            "   settle. Never overload a short shot.\n"
            "Write it as ONE paragraph, present tense, 40-80 words, vivid and specific but grounded in physical \n"
            "detail. Describe what the person DOES in plain positive words - no negative phrasing. Stable, \n"
            "physically-grounded motion reads as sharp and realistic (a busy, unanchored shot reads as blurry).\n\n"
