...
def create_step() -> Step:
    s = Step(
        name="age_normalize",
        desc="Normalize target_age range → single int 18-25 for consistent age in image+video prompts",
        requires=["target_age"],
        outputs=["_normalized_age"],
        help_text="Requires: target_age (e.g. '20-35'). Outputs single int 18-25.",
    )
    async def run(ctx):
