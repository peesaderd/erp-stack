...
resultInfo.textContent = '🔍 พบ ' + analyzed.length + ' รายการ สำหรับ "' + q + '"';
    ...
    if (!analyzed.length) {
      ...
      grid.innerHTML = analyzed.map(p => {
        ...
        return `<div class="product-card"><div class="product-card-checkbox" style="left:5px;top:5px"><input type="checkbox" onchange="toggleBatchSelect(this, ${JSON.stringify(batchData).replace(/'/g,"\\'")})"></div><div class="product-card-title" style="padding:10px 12px 0;font-size:13px;font-weight:700;line-height:1.35;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;word-break:break-word" onclick="useProduct('${enc}')" title="${(p.title||'Unknown').replace(/"/g,'&quot;')}">${p.title || "Unknown"}</div>${imgHtml}<div class="product-card-body" onclick="useProduct('${enc}')" style="cursor:pointer"><div style="display:flex;align-items:center;gap:12px;font-size:12px;color:var(--text-secondary)"><span style="font-weight:800;color:var(--brand);font-size:14.5px">฿${Number(price).toLocaleString()}</span><span class="badge badge-green" style="font-size:10px">💰 ${comm}%</span><span style="font-size:11px">🔥 ขายแล้ว ${sold.toLocaleString()} ชิ้น</span></div><div style="display:flex;align-items:center;gap:6px;margin-top:2px"><span class="badge" style="font-size:10px;background:rgba(255,255,255,0.06)">🥫 ${p.container_type||"ขวดมาตรฐาน"}</span><span class="badge" style="font-size:10px;background:rgba(255,255,255,0.06)">🔒 ${p.closure_type||"ฝามาตรฐาน"}</span></div></div><div style="padding:6px 12px 12px"><button class="btn btn-primary btn-sm" style="width:100%;background:linear-gradient(135deg, var(--brand), var(--accent-purple));border:none" onclick="event.stopPropagation();useProduct('${enc}')">🎬 สร้างคลิป</button></div></div>`;
      }).join("");
    }
...
async function loadAnalyzedProducts() {
  ...
  try {
    ...
    grid.innerHTML = products.map(p => {
      ...
      return `<div class="product-card"><div class="product-card-checkbox"><input type="checkbox" data-enc="${enc}" onchange="toggleBatchSelect(this)"></div><div class="product-card-title" style="padding:10px 12px 0;font-size:13px;font-weight:700;line-height:1.35;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;word-break:break-word" onclick="useProduct('${enc}')" title="${(p.title||'Unknown').replace(/"/g,'&quot;')}">${p.title || "Unknown"}</div>${imgHtml}<div class="product-card-body" onclick="useProduct('${enc}')" style="cursor:pointer"><div style="display:flex;align-items:center;gap:12px;font-size:12px;color:var(--text-secondary)"><span style="font-weight:800;color:var(--brand);font-size:14.5px">฿${Number(price).toLocaleString()}</span><span class="badge badge-green" style="font-size:10px">💰 ${comm}%</span><span style="font-size:11px">🔥 ขายแล้ว ${sold.toLocaleString()} ชิ้น</span></div></div><div style="padding:6px 12px 12px"><button class="btn btn-primary btn-sm" style="width:100%;background:linear-gradient(135deg, var(--brand), var(--accent-purple));border:none" onclick="event.stopPropagation();useProduct('${enc}')">🎬 สร้างคลิป</button></div></div>`;
    }).join("");
