...
resultInfo.textContent = '🔍 พบ ' + analyzed.length + ' รายการ สำหรับ "' + q + '"';
    ...
    if (!analyzed.length) {
      ...
      grid.innerHTML = analyzed.map(p => {
        ...
        return `<div class="product-card"><div class="product-card-checkbox" style="left:5px;top:5px"><input type="checkbox" onchange="toggleBatchSelect(this, ${JSON.stringify(batchData).replace(/'/g,"\\'")})"></div><div style="margin-left:14px">${imgHtml}</div><div class="product-card-body" onclick="useProduct('${enc}')" style="margin-left:4px;cursor:pointer"><div class="product-card-title" style="margin-bottom:4px;font-size:13.5px;font-weight:700">${p.title || "Unknown"}</div><div style="display:flex;align-items:center;gap:12px;font-size:12px;color:var(--text-secondary)"><span style="font-weight:800;color:var(--brand);font-size:14.5px">฿${Number(price).toLocaleString()}</span><span class="badge badge-green" style="font-size:10px">💰 ${comm}%</span><span style="font-size:11px">🔥 ขายแล้ว ${sold.toLocaleString()} ชิ้น</span></div><div style="display:flex;align-items:center;gap:6px;margin-top:2px"><span class="badge" style="font-size:10px;background:rgba(255,255,255,0.06)">🥫 ${p.container_type||"ขวดมาตรฐาน"}</span><span class="badge" style="font-size:10px;background:rgba(255,255,255,0.06)">🔒 ${p.closure_type||"ฝามาตรฐาน"}</span></div></div><div class="product-card-actions" style="margin-left:auto"><button class="btn btn-primary btn-sm" style="background:linear-gradient(135deg, var(--brand), var(--accent-purple));border:none" onclick="event.stopPropagation();useProduct('${enc}')">🎬 สร้างคลิป</button></div></div>`;
      }).join("");
    }
...
async function loadAnalyzedProducts() {
  ...
  try {
    ...
    grid.innerHTML = products.map(p => {
      ...
      return `<div class="product-card"><div class="product-card-checkbox"><input type="checkbox" data-enc="${enc}" onchange="toggleBatchSelect(this)"></div>${imgHtml}<div class="product-card-body" onclick="useProduct('${enc}')"><div class="product-card-title">${p.title || "Unknown"}</div><div class="product-card-meta"><span class="product-card-price">฿${Number(price).toLocaleString()}</span><span class="product-card-comm">💰 ${comm}%</span><span class="product-card-platform">${p.source || "tiktok"}</span></div><div class="product-card-sold">🔥 Sold ${sold.toLocaleString()} pcs ${imgCount ? '· 📸 ' + imgCount + ' รูป' : ''}</div><div class="product-card-footer"><span class="text-xs text-secondary truncate" style="max-width:140px">${pUrl}</span><button class="btn btn-primary btn-sm" onclick="event.stopPropagation();useProduct('${enc}')">🎬 Create Video</button></div></div></div>`;
    }).join("");
